...
export default function CategoriasGastos() {
  ...
  async function onSubmit(datos) {
    ...
    const { error } = editando
      ? await supabase.from('categorias_gastos').update(payload).eq('id', editando.id)
      : await supabase.from('categorias_gastos').insert({ ...payload, comercio_id: perfil.comercio_id })
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <p className="text-sm text-slate-500">{categorias.length} categorías de gastos</p>
        {esAdmin && (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-x-auto">
          <table className="w-full text-sm min-w-[520px]">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {categorias.length === 0 && (
                <tr>
                  <td colSpan={esAdmin ? 4 : 3} className="px-5 py-12 text-center text-slate-400">
                    No hay categorías todavía. {esAdmin && 'Creá la primera.'}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {esAdmin && (
        <Modal
          ...
          titulo={editando ? 'Editar categoría' : 'Nueva categoría de gasto'}
        >
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <Input
              label="Nombre *"
              placeholder="Ej: Servicios, Alquiler, Personal..."
