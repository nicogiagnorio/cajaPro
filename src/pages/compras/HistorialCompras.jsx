...
import {
  ...
  TrendingDown, Calendar, ExternalLink,
} from 'lucide-react'
...
function BadgeRecepcion({ estado, size = 'sm' }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full font-semibold bg-amber-100 text-amber-700 ${cls}`}>
      <Clock size={10} /> Pendiente recepcion
    </span>
  )
}
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          ...
          <div className="space-y-2 pt-1">
            ...
            {/* Marcar recibida — solo detallada pendiente */}
              ...
              <div>
                {!mostrarFactura ? (
                  ...
                  >
                    <span className="flex items-center gap-2">
                      <FileText size={15} />
                      Registrar numero de factura
                    </span>
                    <ChevronDown size={14} className="-rotate-90" />
                  </button>
                ) : (
                  <div className="bg-violet-50 border border-violet-200 rounded-xl px-4 py-3 space-y-3">
                    <p className="text-sm font-medium text-violet-800">Numero de factura</p>
                    <input
                      type="text"
                      autoFocus
...
export default function HistorialCompras() {
  ...
  // Actualizar compra local y en lista
  ...
  // Accion: marcar recibida (actualiza stock para detalladas)
  ...
  return (
    <div className="h-full flex flex-col gap-5 overflow-hidden">
      ...
      {/* Header: KPIs + botones de accion */}
      <div className="flex items-start gap-4 flex-shrink-0">
        {/* KPIs */}
        <div className="flex gap-3 flex-1">
          <div className="bg-white rounded-2xl border border-slate-200 px-5 py-4 flex-1 min-w-0">
        ...
        <div className="flex gap-2 flex-shrink-0">
          ...
          <button
            onClick={() => navigate('/app/compras/detallada')}
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        ...
        <select
          ...
          className="px-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-200 bg-white text-slate-700 cursor-pointer"
        >
          <option value="todos">Recepcion: todas</option>
        ...
        {errorCarga ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-slate-400">
              {compras.length > 0
                ? 'Probas con otros filtros'
                : 'Registra tu primera compra del mes'
              }
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="text-left px-3 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Proveedor / Descripcion</th>
                  <th className="text-right px-3 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Total</th>
                  <th className="text-center px-3 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Recepcion</th>
