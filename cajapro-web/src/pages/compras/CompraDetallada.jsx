...
function ContenidoRecepcion({
  carrito, setCarrito, cambiarCantidad, cambiarPrecio, quitarItem, total,
  proveedor, setProveedor, proveedores,
  facturada, setFacturada, tipoFact, setTipoFact, nroFact, setNroFact, fechaFact, setFechaFact,
  pagada, setPagada, metodoPago, setMetodoPago, fechaPago, setFechaPago,
  error, registrando, registrarCompra,
}) {
  return (
    <>
        ...
        <div className="px-3 py-2 space-y-2">
          {carrito.length === 0 ? (
            <p className="text-center text-slate-400 text-sm py-8">
              Seleccioná los productos recibidos
            </p>
          ) : (
            carrito.map(item => (
...
export default function CompraDetallada() {
  ...
  return (
    <div className="h-full flex gap-5 relative">
      ...
      {/* ── Panel recepción: escritorio ── */}
      <div className="hidden lg:flex w-80 flex-shrink-0 flex-col bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <ContenidoRecepcion {...propsRecepcion} />
      </div>
        ...
        >
          <div className="bg-white rounded-t-2xl w-full max-h-[88vh] flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 flex-shrink-0">
              <p className="font-bold text-slate-900">Recepción</p>
              <button onClick={() => setRecepcionAbierta(false)} className="text-slate-400 hover:text-slate-600">
                <X size={20} />
              </button>
            </div>
