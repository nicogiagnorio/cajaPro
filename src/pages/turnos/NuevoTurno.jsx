...
const PIX_MIN         = 1.5   // píxeles por minuto
const SLOT_SNAP       = 30    // snap a 30 minutos
...
function MiniCalendar({ profesionalId, comercioId, selectedFecha, selectedHoraInicio, selectedHoraFin, onSlotSelect }) {
  ...
  const [hover,    setHover]    = useState(null) // { diaIdx, minutos }
  ...
  const cantidadSemi = (HORA_CAL_FIN - HORA_CAL_INICIO) * 2 // líneas cada 30 min
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden flex flex-col">
      ...
      <div className="overflow-y-auto" style={{ maxHeight: 520 }}>
        <div className="flex" style={{ minWidth: 400 }}>
          ...
          <div className="w-9 flex-shrink-0 border-r border-slate-100 bg-white sticky left-0 z-10">
            <div className="h-9 border-b border-slate-100" /> {/* espacio cabecera */}
      ...
      <div className="px-3 py-1.5 border-t border-slate-100 text-center text-[10px] text-slate-400 flex-shrink-0">
        {profesionalId
          ? 'Clic en un horario libre para pre-completar el formulario'
...
export default function NuevoTurno() {
  ...
  // Llamado desde el calendario cuando se hace clic en un slot
  ...
  // ── Layout split: calendario + formulario ──────────────────────────────
  return (
