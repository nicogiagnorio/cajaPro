...
import {
  ...
  ScanBarcode, Tag, Trash2, Zap, ChevronUp,
} from 'lucide-react'
...
// Contenido del carrito — compartido entre el panel de escritorio y la hoja mobile
function ContenidoCarrito({
  tema, carrito, setCarrito, cambiarPrecio, cambiarCantidad, setCantidadDirecta, quitarItem,
  descuento, setDescuento, tipoDescuento, setTipoDescuento,
  subtotal, descNum, total,
  clientes, clienteId, setClienteId,
  metodoPago, setMetodoPago, efectivoEntregado, setEfectivoEntregado, vuelto,
  error, vendiendo, confirmarVenta,
}) {
  return (
    <>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1">
                  ...
                  <input type="number" min="0.001" step="1"
                    ...
                    className="w-12 text-center text-sm border border-slate-200 rounded-lg py-0.5 focus:outline-none"
                  />
                  <button onClick={() => cambiarCantidad(item.id, +1)}
                    className="w-7 h-7 rounded-lg bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-100 transition-colors">
                    <Plus size={11} />
      ...
      <div className="px-4 py-4 border-t border-slate-100 space-y-3 flex-shrink-0">
        ...
        <Link to="/app/ventas/caja" className="block text-center text-xs text-slate-400 hover:text-slate-600 py-1 transition-colors">
          <Zap size={11} className="inline mr-1" />Ir a venta rápida
        </Link>
      </div>
    </>
  )
...
export default function NuevaVenta() {
  ...
  if (confirmada) {
    return (
      <div className="h-full flex items-center justify-center p-3">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm w-full max-w-sm p-6 sm:p-8 space-y-5">
  ...
  return (
    <div className="h-full flex gap-4 overflow-hidden relative">
                    ...
                    <button
                      ...
                      className={`bg-white border rounded-xl p-4 text-left transition-all group text-sm ${
                        sinStock
                          ...
                          : 'border-slate-200 hover:shadow-md active:scale-95'
                      }`}
                    >
      ...
      {/* ── Panel carrito: escritorio (fijo a la derecha) ── */}
      <div className="hidden lg:flex w-[340px] flex-shrink-0 flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <ContenidoCarrito {...propsCarrito} />
      </div>

      {/* ── Barra flotante: mobile/tablet ── */}
      {carrito.length > 0 && !carritoAbierto && (
        ...
        >
          ...
          <span className="text-sm font-semibold flex-1 text-left">
            {carrito.reduce((s,i) => s + i.cantidad, 0)} item{carrito.length !== 1 ? 's' : ''}
          </span>
        ...
        >
          <div className="bg-white rounded-t-2xl w-full max-h-[88vh] flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 flex-shrink-0">
              <p className="font-bold text-slate-900">Tu venta</p>
              <button onClick={() => setCarritoAbierto(false)} className="text-slate-400 hover:text-slate-600">
                <X size={20} />
              </button>
            </div>
